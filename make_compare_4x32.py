"""4x: Bicubic | WebVSR 4x 16ch | WebVSR 4x 32ch | Ground Truth (compressed input)."""
...
picks = [files[i] for i in [3, 12, 24] if i < len(files)]
CROP = 240
rows = []
for f in picks:
    ...
    hr_t = to_t(hr).to(DEV)
    lr = degrade_second_order(hr_t.squeeze(0).cpu(), scale=4).unsqueeze(0).to(DEV).clamp(0,1)
    bic = F.interpolate(lr, size=hr_t.shape[2:], mode="bicubic", align_corners=False).clamp(0,1)
    with torch.no_grad():
        s16 = m16(lr).clamp(0,1); s32 = m32(lr).clamp(0,1)
    ...
    cols = [to_im(bic), to_im(fit(s16)), to_im(fit(s32)), hr]
    w,h = cols[0].size
...
ImageDraw.Draw(canvas).text((8,6),"Bicubic  |  4x 16ch  |  4x 32ch  |  Ground Truth", fill=(210,210,210))
y=24
